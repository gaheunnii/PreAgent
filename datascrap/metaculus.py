...
from utils import api_utils, data_utils, time_utils, data_scraping, information_retrieval
...
params = {"limit": 100}
METACULUS_API_URL = "https://www.metaculus.com/ap/"
...
def setup_driver():
    ...
    chrome_options.add_argument("--disable-gpu")  # 禁用 GPU（仅限非必要时）
...
def process_questions(questions, driver):
    ...
    """
    question_ids = [questions[i]["post_id"] for i in range(len(questions))]
...
def fetch_all_valid_questions(base_url, headers, params):
    ...
    for question in valid_questions:
        question["close_time"] = time_utils.safe_to_datetime(question["actual_close_time"])
        question["is_resolved"] = True if question["status"].lower() == "resolved" else False
    ...
    for q in valid_questions:
        if q["is_resolved"] and str(q['close_time'].date())>='2024-05-01' and str(q['close_time'].date())<='2025-04-01':
            questionfilter.append(q)
...
def main(n_days=None):
    ...
    if n_days is not None:
        ...
        questions_url = METACULUS_API_URL + "questions/?include_description=true&publish_time__gt=2024-05-01&publish_time__lt=2025-04-01"
        comments_url = METACULUS_API_URL + "comments/?created_time__gt=2024-05-01&created_time__lt=2025-04-01"
    ...
    data_dict = {d["post_id"]: d for d in data if d["post_id"]}
