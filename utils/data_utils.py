...
from utils import db_utils, time_utils, string_utils, model_eval
...
def get_formatted_data(
    ...
    data=None,
    root_dir=keys.get("DATA_ROOT_DIR", "./data")
):
    """
    ...
    if not data:
        try:
            ...
            with open(os.path.join(root_dir,s3_path),'rb') as f:
                data=pickle.load(f)
    ...
    for q in data:
        ...
        # print(q['is_resolved'])
        # print(q['i18n'])
        # print(q['suggesting_resolution'])
        # print(q['data_source'])
        # print(q['question'].keys())
        # print(q['question']['ends_at'])
        # print(q['question']['name'])
        # print(q['question']['description'])
        # print(q['question']['resolution_notes'])
        # print(q['question']['resolved?'])
        # print(q['question']['resolved_at'])
        # print(q['question']['state'])
        # print(q['question']['type'])
        ...
        for key, value in {
            ...
            "category_list": q["gpt_4o_category"],
        }.items():
...
def assign_ill_defined_questions(data_list, model_name="gpt-4o"):
    """
...
def assign_categories(data_list, model_name="gpt-4o"):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_question = {
            ...
            for item in data_list
            if "gpt_4o_category" not in item
        }
        ...
        for future in concurrent.futures.as_completed(future_to_question):
            ...
            try:
                result = future.result()
                if result is not None:
                    question_item["gpt_4o_category"] = result
                else:
                    logger.warning(
