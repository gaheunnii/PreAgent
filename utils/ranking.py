...
from configs.constants import CHARS_PER_TOKEN, DEFAULT_RETRIEVAL_CONFIG
from utils import information_retrieval,model_eval,summarize
...
async def rank_articles(
    ...
    if method == "llm-rating":
        ...
        q_embedding, a_embeddings = get_question_article_embeddings(
            articles, question, background,domain_ref
        )
...
def get_question_article_embeddings(articles, question, background,domain_ref):
    """
    ...
    q_embedding = model_eval.get_openai_embedding(
        [
            "Question: {question}\n\nBackground:{background}\n\n Domain Refer:{domain_ref}".format(
                question=question, background=background,domain_rer=domain_ref
            )
        ]
    )
...
async def retrieve_summarize_and_rank_articles(
    ...
    if config.get("PRE_FILTER_WITH_EMBEDDING") and len(articles) >= 25:
        ...
        cos_sim = []
        q_embedding, a_embeddings = get_question_article_embeddings(
            articles, question, background_info,domain_ref
        )
        # each a_embedding is an Embedding object, with a_embedding.embedding being a list of floats
    ...
    logger.info(f"noranked articles number: {len(articles)}")
    ...
    logger.info(f"ranked articles number: {len(ranked_articles)}")
