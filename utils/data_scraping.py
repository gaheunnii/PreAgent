...
# from configs.constants import S3, S3_BUCKET_NAME
# from utils import db_utils

logger = logging.getLogger(__name__)


def save_data_structure_to_local(data_structure, s3_path,root_dir=keys.get("DATA_ROOT_DIR", "./data")):
    dir_name = os.path.dirname(os.path.join(root_dir,s3_path))
    os.makedirs(dir_name,exist_ok=True)

    with open(os.path.join(root_dir,s3_path), "wb") as f:
...
def upload_scraped_data(data, source, question_types, n_days_or_not=None):
    ...
    for q_type in question_types:
        ...
        save_data_structure_to_local(questions, s3_path)
