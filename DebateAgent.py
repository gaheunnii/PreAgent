...
def get_log_filename(args):
    ...
    return f'logs/debate_{args.dataset}_r{args.max_rounds}_t{args.temperature}_s{args.seed}_{current_time}.log'
...
class ModeratorAgent(RoutedAgent):
    ...
    async def start_debate(self, message: DebateQuestion, ctx: MessageContext) -> None:
        ...
        await self.publish_message(
            message, 
            topic_id=DefaultTopicId(type='debate_question')
        )
        ...
        await self.publish_message(
            ...
            topic_id=DefaultTopicId(type='debate_turn')
        )
    ...
    async def handle_argument(self, message: DebateArgument, ctx: MessageContext) -> None:
        ...
        # # logger.info(f"Received {len(current_round_args)} arguments for Round {message.round}")
        # # logger.info(f"{self.current_round}")
        # # logger.info(self.debate_history)
        # # logger.info(current_round_args)
        ...
        #     # logger.info(f"YES side argument:\n{yes_arg.argument}")
        #     # logger.info(f"NO side argument:\n{no_arg.argument}")
        yes_arg = next((arg for arg in self.debate_history if arg.round == message.round and arg.side == "yes"), None)
        no_arg = next((arg for arg in self.debate_history if arg.round == message.round and arg.side == "no"), None)
        
        logger.info(f"Round {message.round}: YES argument received: {yes_arg is not None}")
        logger.info(f"Round {message.round}: NO argument received: {no_arg is not None}")

        # 只有当两方论点都收到时才进行处理
        if yes_arg is not None and no_arg is not None:
            # 确保当前轮次与消息轮次一致
            ...
            await self.publish_message(
                round_summary,
                topic_id=DefaultTopicId(type='debate_summary')
            )
    ...
    async def make_final_decision(self, ctx: MessageContext) -> None:
        ...
        await self.publish_message(
            final_result,
            topic_id=DefaultTopicId(type='debate_result')
        )
...
class DebaterAgent(RoutedAgent):
    ...
    @message_handler
    async def debate_turn(self, message: DebateArgument, ctx: MessageContext) -> None:
        """轮到该辩论者发言"""
        if message.side != "request":
            if message.side != self.side:
                self.opponent_arguments.append(message)
            # logger.info(f"=======Debater {self.side.upper()}: Received {message.side.upper()} Argument=======")
        ...
        if '(' in info_to_retrieve:
            info_to_retrieve=info_to_retrieve.split('(')[1].strip()
        if ')' in info_to_retrieve:
            info_to_retrieve=info_to_retrieve.split(')')[0].strip()
        ...
        # logger.info(f"{self.type}")
        # logger.info("=======Debater: End of Turn=======")
        await self.publish_message(
            DebateArgument(side=self.side,
            argument=argument,
            evidence=evidence,
            round=current_round),
            topic_id=DefaultTopicId(type='debate_turn')
        ...
        # )
# 主函数
async def run_debate(question: DebateQuestion, max_rounds: int = 3) -> DebateResult:
    """运行辩论"""
    ...
    await runtime.add_subscription(TypeSubscription(topic_type="debate_turn", agent_type="moderator"))
